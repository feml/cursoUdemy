...
#pragma warning disable CA2100 // Review SQL queries for security vulnerabilities
                OracleCommand cmd = new OracleCommand(SelectCommand.GetCommand(select, _nParametros, _vParametros), conn);
#pragma warning restore CA2100 // Review SQL queries for security vulnerabilities
                ...
                catch (OracleException ex)
                {
                    ...
                    PrcsOracleException(SelectCommand.GetCommand(select, _nParametros, _vParametros), ex);
                }
                catch (Exception ex)
                {
                    if (conn.State == ConnectionState.Open)
                    ...
                    PrcsException(SelectCommand.GetCommand(select, _nParametros, _vParametros), _nParametros, _vParametros, ex);
                }
                finally
                {
                    if (conn.State == ConnectionState.Open)
...
#pragma warning restore CA2100 // Review SQL queries for security vulnerabilities
        ...
        {
            object[] values = { valor };
            string tipoObject = string.Empty;
            //bool continua = true;
            OracleParameter op = new OracleParameter();
            op.ParameterName = pnombre;
            op.Direction = ParameterDirection.Input;
            foreach (var value in values)
                tipoObject=value.GetType().Name;
            switch (tipoObject)
            {
                case "DateTime":
                    {
        ...
        private void PrcsException(string select, string[] _nParametros, object[] _vParametros, Exception ex)
        {
            DataTable dtTmp = new DataTable();
            dtTmp.Columns.Add("Error", typeof(string));
            dtTmp.Rows.Add(ex.Message);
            logException(SelectCommand.GetCommand(select, _nParametros, _vParametros), dtTmp);
        }
